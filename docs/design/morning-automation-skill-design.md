...
### 3.1 Daily 外层编排（09:00 -> 09:20，分步汇报）
...
2. Step 1 完成后，立即发送独立消息 `Step 1 Report`：
   ...
   - 消息末尾固定声明：`接下来将开始执行 Step 2 (AI 日报 + 前沿更新)`。
3. Step 1 消息发送后，无需人工确认，自动启动 Step 2 子 Agent。
4. 启动 Step 2 子 Agent：调用 `ai-daily-news-and-changelog`。
5. Step 2 完成后，立即发送独立消息 `Step 2 Report`（包含完整结果）。
6. Step 2 汇报后，立即发送 `Final Global Summary`：
   - overall 状态（`SUCCESS` / `PARTIAL` / `FAILED`）
   - Step 1 与 Step 2 的结果归纳
   - 失败或异常时的建议动作
...
daily 编排输出不是单条消息，而是固定三段消息序列：

1. `Step 1 Report`（小桔签到结果 + 下一步声明）
2. `Step 2 Report`（AI 日报结果）
3. `Final Global Summary`（总体状态 + 分步状态 + 建议动作）
